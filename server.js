...
app.get('/auth/alexa/callback', async (req, res) => {
  ...
  try {
    ...
    if (!req.sessionID) {
      console.error('❌ No session ID - generating new session');
    ...
    async function proceedWithTokenExchange() {
      console.log('🔑 Exchanging code for tokens for session:', req.sessionID);
